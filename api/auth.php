...
if($_SERVER['REQUEST_METHOD']==='POST' && $action==='register'){
 ...
 try{
  ...
  out(['user'=>['id'=>$id,'name'=>$name,'email'=>$email,'phone'=>null,'balance_cents'=>0,'has_payment_profile'=>0,'has_shipping_profile'=>0,'role'=>'customer','allow_postpaid'=>0]]);
...
if($_SERVER['REQUEST_METHOD']==='POST' && $action==='login'){
 ...
 $q=$pdo->prepare('SELECT id,name,email,phone,password_hash,status,role,allow_postpaid,(pagarme_customer_id IS NOT NULL) AS has_payment_profile,EXISTS(SELECT 1 FROM user_shipping_profiles sp WHERE sp.user_id=users.id) AS has_shipping_profile FROM users WHERE email=? LIMIT 1');$q->execute([$email]);$u=$q->fetch();
 if(!$u || $u['status']!=='active' || empty($u['password_hash']) || !password_verify($pass,$u['password_hash'])) out(['error'=>'E-mail ou senha incorretos'],401);
 $pdo->prepare('INSERT IGNORE INTO wallets(user_id,balance_cents) VALUES(?,0)')->execute([(int)$u['id']]);
 ...
 out(['user'=>['id'=>(int)$u['id'],'name'=>$u['name'],'email'=>$u['email'],'phone'=>$u['phone'],'balance_cents'=>(int)$q->fetchColumn(),'has_payment_profile'=>(int)$u['has_payment_profile'],'has_shipping_profile'=>(int)$u['has_shipping_profile'],'role'=>$u['role'],'allow_postpaid'=>(int)$u['allow_postpaid']]]);
}

if($_SERVER['REQUEST_METHOD']==='POST' && $action==='logout'){$_SESSION=[];session_destroy();out(['ok'=>true]);}
if($action==='me'){
 $id=userId();$q=$pdo->prepare('SELECT u.id,u.name,u.phone,u.email,u.role,u.allow_postpaid,(u.pagarme_customer_id IS NOT NULL) AS has_payment_profile,EXISTS(SELECT 1 FROM user_shipping_profiles sp WHERE sp.user_id=u.id) AS has_shipping_profile,COALESCE(w.balance_cents,0) AS balance_cents FROM users u LEFT JOIN wallets w ON w.user_id=u.id WHERE u.id=?');$q->execute([$id]);out(['user'=>$q->fetch()]);
}
